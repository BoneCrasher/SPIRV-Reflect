...
int main(int argn, char** argv)
{
  ...
  result = spvReflectCreateShaderModule(sizeof(k_sample_spv), k_sample_spv, &module);
  assert(SPV_REFLECT_RESULT_SUCCESS == result);
  ...
  for (size_t i_set = 0; i_set < sets.size(); ++i_set)
  {
    ...
    {
      SpvReflectDescriptorBinding const &refl_binding = *(refl_set.bindings[i_binding]);
